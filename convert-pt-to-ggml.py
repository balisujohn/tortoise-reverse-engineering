...
try:
    ...
    with io.BytesIO(model_bytes) as fp:
        ...
        print(checkpoint.heads)
except Exception:
    print("Error: failed to load PyTorch model file:" , fname_inp)
    sys.exit(1)

auto_conditioning_tensor = torch.load("auto_conditioning.pt")
#print(auto_conditioning_tensor)
#exit()
...
list_vars["auto_conditioning"] = auto_conditioning_tensor
...
fout.write(struct.pack("i", checkpoint.max_mel_tokens))
fout.write(struct.pack("i", checkpoint.max_text_tokens))
fout.write(struct.pack("i", checkpoint.max_conditioning_inputs))
fout.write(struct.pack("i", checkpoint.layers))
fout.write(struct.pack("i", checkpoint.model_dim)) # also, embedding dim
fout.write(struct.pack("i", checkpoint.heads))
fout.write(struct.pack("i", checkpoint.number_text_tokens))
fout.write(struct.pack("i", checkpoint.start_text_token))
fout.write(struct.pack("i", checkpoint.text_embedding.num_embeddings))
...
for name in list(list_vars.keys()):
    export_list = ["text_embedding.weight", "text_pos_embedding.emb.weight",
    "mel_embedding.weight", "mel_pos_embedding.emb.weight"]
    if not name in export_list and not("inference_model" in name and ".h.0." in name):
        continue



    #if name != "text_embedding.weight" and name != "text_pos_embedding.emb.weight":
    #    continue
    data = list_vars[name].squeeze().numpy()
    print("Processing variable: " , name ,  " with shape: ", data.shape)

    # reshape conv bias from [n] to [n, 1]
    if name in ["conditioning_encoder.init.bias"]:
...
#save the auto conditioning tensor
data = auto_conditioning_tensor.numpy()
n_dims = len(data.shape)

name = "conditioning_latent"
print("Processing variable: " , name ,  " with shape: ", data.shape)
str_ = name.encode('utf-8')
